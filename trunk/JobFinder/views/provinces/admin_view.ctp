...
<?php endforeach; ?>
	...
	<?php
        echo $this->Form->input('country_id',array('label'=>'Quốc gia:','div'=>false,'empty'=>'Vui lòng chọn...','class'=>'block','error'=>array('wrap'=>'span'),'disabled'=>true));
		echo $this->Form->input('name',array('label'=>'Tỉnh/thành phố:','div'=>false,'error'=>array('wrap'=>'span'),'disabled'=>true));
	?>
		<div align="center">
        <br/>
	    	<?php echo $this->Html->link(__('Thêm mới', true),array('action' => 'index'));?> | 
	    	<?php echo $this->Html->link(__('Chỉnh sửa', true), array('action' => 'edit', $this->data['Province']['id'])); ?>
	    </div>
	</div>
	</div>
</div>